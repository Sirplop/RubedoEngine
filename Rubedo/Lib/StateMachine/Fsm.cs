...
using System.Text;
using Rubedo.Lib.StateMachine.Events;
using Rubedo.Lib.StateMachine.Fluent;

namespace Rubedo.Lib.StateMachine;
...
public class Fsm<TS, TT>
{
	...
	public void JumpTo(TS state, bool isPop = false)
	{
		...
		{
			DoTransition(state, default, isPop);
		}
	}
	...
	private bool CheckAfterEntries(List<Timer<TS>> afterEntries,
		Dictionary<TS, Transition<TS, TT>> transitions, double timeSpan)
	{
		for (var i = 0; i < afterEntries.Count; i++)
		{
			...
			double? r = e.Tick(timeSpan);
			afterEntries[i] = e;

			if (!r.HasValue) continue;

			// It triggered.
			DoTransition(e.Target, default, t.Pop);
			Update(timeSpan - timerMax);
			return true;
		}
